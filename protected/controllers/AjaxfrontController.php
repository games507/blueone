...
class AjaxfrontController extends CController
{
	...
	{		
		if ( $res=Driver::getTaskByToken($this->data['task_id'])){				
			...
			$driver_lat = isset($res['driver_location_lat'])?$res['driver_location_lat']:'';
			$driver_lng = isset($res['driver_location_lng'])?$res['driver_location_lng']:'';
			
			$to_lat = ''; $to_lng='';
			
			if(!empty($driver_lat) && !empty($driver_lng)){
				switch ($res['trans_type']) {
					case "delivery":
						if(!empty($res['drop_address'])){
							$to_lat = $res['dropoff_task_lat'];
							$to_lng = $res['dropoff_task_lng'];
						} else {
							$to_lat = $res['task_lat'];
							$to_lng = $res['task_lng'];
						}
			...
			if(!empty($driver_lat) && !empty($driver_lng) && !empty($to_lat) && !empty($to_lng)){
				...
				try {
				   ...
				   $resp_disntance = MapsWrapper::getDistance(
					 $driver_lat,$driver_lng,$to_lat,$to_lng,'M',$travel_mode
				   );				   
				   $distance = $resp_disntance['duration'];
				   
				   MapsWrapper::saveLogs($provider,'getDistance',$resp_disntance);
				} catch (Exception $e) {
				    MapsWrapper::saveLogs($provider,'getDistance', $e->getMessage() );
				}
			}
