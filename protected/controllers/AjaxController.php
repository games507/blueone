...
class AjaxController extends CController
{
	...
	public function actionAddTask()
	{
		...
		if($Validator->validate()){
			...
			$params_recurrent = array();
			$params_recurrent = array(
			  'recurrent_task_enabled'=>isset($this->data['recurrent_task_enabled'])?(integer)$this->data['recurrent_task_enabled']:0,
			  'recurrent_task_start_date'=>isset($this->data['recurrent_task_start_date'])?$this->data['recurrent_task_start_date']:NULL,
			  'recurrent_task_end_date'=>isset($this->data['recurrent_task_end_date'])?$this->data['recurrent_task_end_date']:NULL,					  
			);			
						
			$rr_days = Driver::returnDays();			
			
			$recurrent_task_days = isset($this->data['recurrent_task_days'])?$this->data['recurrent_task_days']:'';			
			$recurrent_task_repetition = isset($this->data['recurrent_task_repetition'])?$this->data['recurrent_task_repetition']:'';
			
			$recurrent_task_enabled = isset($this->data['recurrent_task_enabled'])?$this->data['recurrent_task_enabled']:'';			
			
			if($recurrent_task_enabled==1):
				foreach ($rr_days as $day) {
					if(in_array($day, (array) $recurrent_task_days )){
						$params_recurrent[$day]=1;
					} else $params_recurrent[$day]=0;
				}
				
				$rr_repetition = Driver::returnTaskRepetition();
				foreach ($rr_repetition as $rr_repetition_val) {
					if(in_array($rr_repetition_val, (array)$recurrent_task_repetition )){
						$params_recurrent[$rr_repetition_val]=1;
					} else $params_recurrent[$rr_repetition_val]=0;
				}
			endif;
			...
			if(is_numeric($this->data['task_id'])){
				...
				if ( $DbExt->updateData("{{driver_task}}",$params,'task_id',$this->data['task_id'])){
					...
					if(method_exists('Driver','insertRecurrentTask') && $recurrent_task_enabled == 1 ){
					   Driver::insertRecurrentTask($this->data['task_id'],$params_recurrent);
					}
				...
				if($DbExt->insertData("{{driver_task}}",$params)){
					...
					if(method_exists('Driver','insertRecurrentTask') && $recurrent_task_enabled == 1 ){			   
					   Driver::insertRecurrentTask($task_id,$params_recurrent);
					}
	...
	{		
		...
		if (isset($this->data['id'])){
			if ( $res=Driver::getTaskId($this->data['id'])){
				...
				if(method_exists('Driver','insertRecurrentTask')){
				   $recurrent_html='';
				   $res['recurrent_task_enabled']  = isset($res['recurrent_task_enabled'])?$res['recurrent_task_enabled']:'';
				   if($res['recurrent_task_enabled']==1){
				   	  
				   	$recurrent_html.='<div class="row">';
				   	
				  	  $recurrent_html.='<div class="col-md-6">';
				   	  $recurrent_html.='<div class="form-group left-form-group">';
			            $recurrent_html.='<label class="font-medium">'.t("Start Date").' :</label>';
			            $recurrent_html.='<p class="v">'.AdminFunctions::prettyDate($res['recurrent_task_start_date']).'</p>';
			          $recurrent_html.='</div>';
			          $recurrent_html.='</div>';
			          
			          $recurrent_html.='<div class="col-md-6">';
			          $recurrent_html.='<div class="form-group left-form-group">';
			            $recurrent_html.='<label class="font-medium">'.t("End Date").' :</label>';
			            $recurrent_html.='<p class="v">'.AdminFunctions::prettyDate($res['recurrent_task_end_date']).'</p>';
			          $recurrent_html.='</div>';
			          $recurrent_html.='</div>';
			          
			         $recurrent_html.='</div>'; 
				   	
				   	  $days = Driver::returnDays();				   	  
				   	  $recurrent_html.='<div class="form-group left-form-group">';
			            $recurrent_html.='<label class="font-medium">'.t("Days to create task").'  :</label>';
			            $recurrent_html.='<ul class="list_inline">';
					   	  foreach ($days as $day) {
					   	  	  if($res[$day]==1){					   	  	  	  
					   	  	  	  $recurrent_html.="<li><i class=\"ion-checkmark-round\"></i>".ucwords($day)."</li>";					   	  	  	  
					   	  	  }
					   	  }				   	  
					   	  $recurrent_html.='</ul>';
				   	  $recurrent_html.='</div>';
				   	  
				   	  $task_repeat = Driver::returnTaskRepetition();				   	  
				   	  $recurrent_html.='<div class="form-group left-form-group">';
			            $recurrent_html.='<label class="font-medium">'.t("Task Repetition").'  :</label>';
			            $recurrent_html.='<ul class="list_inline">';
					   	  foreach ($task_repeat as $task_repeat_val) {
					   	  	  if($res[$task_repeat_val]==1){					   	  	  	  
					   	  	  	  $recurrent_html.="<li><i class=\"ion-checkmark-round\"></i>".ucwords($task_repeat_val)."</li>";					   	  	  	  
					   	  	  }
					   	  }				   	  
					   	  $recurrent_html.='</ul>';
				   	  $recurrent_html.='</div>';
				...
				$res['recurrent_html']=$recurrent_html;
				//dump($recurrent_html);
				/*dump($res);
				die();*/
	...
	{		
		if(isset($this->data['task_id'])){						
			...
			if( Driver::deleteTask($this->data['task_id'])){
				if(method_exists('Driver','deleteRecurrentTask')){
				   Driver::deleteRecurrentTask($this->data['task_id']);
				}
				$this->code=1;
				$this->msg="OK";
	...
	{				
	    ...
	    try {					
			...
			$resp = MapsWrapper::geoCodeAdress($country_name);						
			
			Yii::app()->functions->updateOption('drv_default_location_lat',
			  isset($resp['lat'])?$resp['lat']:'',
			  Driver::getUserId()
			);
			
			Yii::app()->functions->updateOption('drv_default_location_lng',
			  isset($resp['long'])?$resp['long']:'',
			  Driver::getUserId()
			);
	    	
	    	MapsWrapper::saveLogs($provider,'geoCodeAdress',$resp);
		} catch (Exception $e) {			
	...
	{						
		...
		if(Yii::app()->db->createCommand()->insert("{{push_broadcast}}",$params)){	
			$this->code=1;
