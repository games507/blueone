...
class AppController extends CController
{
	...
	{		
		...
		if($driver_activity_tracking_interval<=0){
			$driver_activity_tracking_interval=60;
		}
		...
		$rr_days = json_encode(Driver::returnDays());
		$cs->registerScript(
		  'rr_days',
		 "var rr_days='".CJavaScript::quote($rr_days)."';",
		  CClientScript::POS_HEAD
		);
		
		$rr_repetion = json_encode(Driver::returnTaskRepetition());
		$cs->registerScript(
		  'rr_repetion',
		 "var rr_repetion='".CJavaScript::quote($rr_repetion)."';",
		  CClientScript::POS_HEAD
		);
	...
	{		
		$msg=''; $error=0; $line_processing=array(); $params=array();
		$params_data=array();
		...
		if (isset($_POST) && $_SERVER['REQUEST_METHOD']=='POST'){		
			...
			if (preg_match("/.csv/i",$filename)) {
				...
				while (($data = @fgetcsv($handle)) !== FALSE){					
					$line_processing[$x]=t("Processing line")." ($x)";
					...
					if ( count($data) >= 7){						
					    ...
					    if (isset($data[7])){
					    	...
					    	if(!empty($data[6])){					    	  
					    	   try {					
									$lat_lng = MapsWrapper::geoCodeAdress($data[6]);									
									$params['task_lat']=$lat_lng['lat'];
					    	        $params['task_lng']=$lat_lng['long'];
					    	        
					    	        MapsWrapper::saveLogs($provider,'geoCodeAdress', $lat_lng );
								} catch (Exception $e) {
