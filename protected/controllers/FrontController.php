...
class FrontController extends CController
{
	...
	public function actionpaymentpsk()
	{
		$hash=isset($_GET['hash'])?$_GET['hash']:'';
		if($res=FrontFunctions::getCustomerByToken($_GET['hash'])){
			...
			$code=AdminFunctions::generateCode(10);
			$reference=$hash."-$code";
			...
			$params = array(
	    	  ...
	    	  'email'=>$email_address,
	    	  'reference'=>$reference,
	    	  'callback_url'=>websiteUrl()."/psk_success"
	    	);	
	    	/*dump($params);	 
	    	die();*/
	    	...
	    	try {
	    		...
	    		$resp = PaystackWrapper::pay($params);
		    	...
		    	$db=new DbExt();
		    	$db->insertData("{{paystack_logs}}",$params_logs);		    
	...
	public function actionpsk_success()
	{ 
		$payment_code='psk';
		$data=$_GET; $db = new DbExt(); $error='';
		$reference = isset($data['reference']) ? $data['reference'] : '';		
		$reference_1 = !empty($reference)?explode("-",$reference):false;
		
		if(is_array($reference_1) && count($reference_1)>=1){
			$hash=$reference_1[0];
			if($res=FrontFunctions::getCustomerByToken($hash)){
				
				try {
			
				    $resp = PaystackWrapper::verifyPayment($reference);					
			        ...
			        $currency_code=FrontFunctions::getCurrenyCode(true);				        
			         ...
			         if($res['renew_plan_id']>0){
			         	...
			         	if ($plan_details=FrontFunctions::getPlansByID($res['renew_plan_id'])){
							...
							$memo="Renew Payment by ".$res['first_name']." ".$res['last_name'];					
							FrontFunctions::savePaymentLogs($customer_id,
							  'signup',$payment_code,$memo,
							  $price,
							  $currency_code,
		...
		} else $error=t("invalid reference number");		
	}
	
	public function actionpaystack_webhook()
	{
		$code = 2; $message = '';
		...
		if(isset($event->obj->event)){
		   switch($event->obj->event){
		   	   case 'charge.success':
		   	   	  $reference = $event->obj->data->reference;
		   	   	  $transaction_id = $event->obj->data->id;		   	   	  
		   	   	  $reference_1=explode("-",$reference);
		   	   	  if(is_array($reference_1) && count($reference_1)>=1){
		   	   	  	 $hash=$reference_1[0];
		   	   	  	 if($res=FrontFunctions::getCustomerByToken($hash)){
		   	   	  	 	$customer_id = $res['customer_id'];		   	   	  	 	
		   	   	  	 	...
		   	   	  	 	{{payment_logs}}
		   	   	  	 	WHERE
		   	   	  	 	transaction_ref=".AdminFunctions::q($transaction_id)."
		   	   	  	 	";		   	   	  	 	
		   	   	  	 	if(!$resp = $db->rst($stmt)){
		   	   	  	 		if($res['renew_plan_id']>0){
								    ...
								    $message="renew successful";
		   	   	  	 			...
		   	   	  	 			} else $message=t("Payment is successful but cannot find plan details");
		   	   	  	 	...
		   	   	  	 	} else $message = "payment already process";
		   	   	  	 	
		   	   	  	 	
		   	   	  	 } else $message = "customer not found";
		   	   	  } else $message = "invalid reference number";
		   	   	  
		   	   	break;
		   	   	
		   	   default:
		        $message ="invalid event ".$event->obj->event;
		       break;
		   }
		} else $message = "receive data not object";
		
		
		$db->insertData("{{paystack_webhook}}",array(
		 'code'=>$code,
		 'message'=>$message,
		 'receive_data'=>$event->raw,
		 'date_created'=>AdminFunctions::dateNow(),
		 'ip_address'=>$_SERVER['REMOTE_ADDR']
		));		
	}
