...
class AdminController extends CController
{
	...
	{
		$this->render('mobile-settings',array(
		  'json'=>getOptionA('services_json')
		));
	}
	...
	public function actiontest_map_api()
	{
		...
		$this->pageTitle = t("Test Map");
		
		$message = ''; $resp = array(); $lat = ''; $lng='';
		
		$map_provider = MapsWrapper::getMapProvider();
		$map_api = isset($map_provider['map_api'])?$map_provider['map_api']:'';
		$provider = isset($map_provider['provider'])?$map_provider['provider']:'';		
		
		try {					
			MapsWrapper::init($map_provider);
			$resp = MapsWrapper::geoCodeAdress("los angeles california");			
			$lat = $resp['lat']; $lng = $resp['long']; 
			MapsWrapper::saveLogs($provider,'geoCodeAdress',$resp);
		} catch (Exception $e) {
			$message = $e->getMessage();
			MapsWrapper::saveLogs($provider,'geoCodeAdress',$message);
		}					
		
		if($provider=="google"):
			Driver::registerJS(array(
			  '//maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&key='.urlencode($map_api),
			  Yii::app()->baseUrl . '/assets/gmaps.js',
			  Yii::app()->baseUrl . '/assets/map_wrapper.js',
			));
		else :
		   Driver::registerJS(array(			  
		   ...
		   Driver::registerCSS(array(
		      Yii::app()->baseUrl."/assets/leaflet/leaflet.css"
		   ));
		endif;
		
		Driver::registerScript(array(
		  "var map_provider= '". CJavaScript::quote($provider) ."';",
		  "var map_lat= '". CJavaScript::quote($lat) ."';",
		  "var map_lng= '". CJavaScript::quote($lng) ."';",
		  "var map_div= '". CJavaScript::quote('map') ."';",
		  "var map_token= '". CJavaScript::quote($map_api) ."';",
		));
		...
		$this->render('test_map_api',array(
		  'provider'=>$provider,
		  'resp'=>$resp,
		  'message'=>$message
		));
