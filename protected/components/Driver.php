...
class Driver
{
	...
	{		
		...
		driver_id=".self::q($driver_id)."		
		AND		
		CAST(delivery_date as DATE) BETWEEN '$start' AND '$end'
		";						
   ...
   public static function availableLanguages()
	...
	{				
		...
		$customer_id = isset($info['customer_id'])? (integer) $info['customer_id']:0;
		...
		$driver_send_push_to_online=getOption( $customer_id , 'driver_send_push_to_online');		
		...
		$website_title = getOptionA('company_name');
				
		//PUSH
		$key_value=getOption( $customer_id ,$key."_PUSH");						
		if ($key_value==1 && $info['enabled_push']==1){
			$push_message=getOption( $customer_id ,$key."_PUSH_TPL");			
			...
			$params=array(
			  ...
			  'push_title'=>Driver::t( strtolower(str_replace("_",' ',$key)) ),
			  'push_message'=>$push_message,
			  'actions'=>$key,			  
			  'driver_id'=>isset($info['driver_id'])? (integer) $info['driver_id']:0,
			  'task_id'=>isset($info['task_id'])? (integer) $info['task_id']: 0,
		...
		//SMS		
		if(self::canCustomerSendSMS($customer_id)){
		...
		if ($key_value==1 && $info['driver_email']!=""){
		   ...
		   $subject = Driver::t( strtolower(str_replace("_",' ',$key)) );

		   $params = array(
		    'email_address'=>trim($info['driver_email']),
		    'subject'=>$subject,
	...
	public static function sendCustomerNotification($key='', $info='' )
	{
		...
		if($key_value==1 && !empty($info['contact_number'])){					
			...
			if(!empty($sms_message)){
				...
				Yii::app()->db->createCommand()->insert("{{sms_logs}}",array(
				  'to_number'=>$info['contact_number'],
				  'sms_text'=>$sms_message,
				  'date_created'=>AdminFunctions::dateNow(),
				  'ip_address'=>$_SERVER['REMOTE_ADDR'],
				  'customer_id'=>(integer)$customer_id
   ...
   public static function driverDropDownList($customer_id='')
	...
	public static function getOfflineDriver($user_id='')
	{
		...
		$offline_interval_date1 = date("Y-m-d H:i:s", strtotime("-10 minutes"));
		$offline_interval_date2 = date("Y-m-d H:i:s", strtotime("-30 minutes"));
		
		$and='';
		if ( $tracking_type==2){															
			//$and.=" AND last_online <='$time_30less' ";					
			$and.=" AND ".q($offline_interval_date1)." < last_login ";
		} else {					
			//$and.=" AND last_online <='$time_now' ";										
			$and.=" AND ".q($offline_interval_date2)." < last_login ";
		}
	...
	public static function returnTaskRepetition()
	{
		return array( 
		  'weekly','monthly','quarterly','half_year','yearly'
		);
	}
	
	public static function insertRecurrentTask($task_id='', $params=array())
	{
		...
		$params['created_at']= AdminFunctions::dateNow();
		$params['updated_at']= AdminFunctions::dateNow();
		$params['ip_address']= $_SERVER['REMOTE_ADDR'];
		
		
		if($res = $db->rst($stmt)){
			unset($params['created_at']);
			$db->updateData("{{recurrent_task}}",$params,'task_id',$task_id);
		} else {
			unset($params['updated_at']);
			$db->insertData("{{recurrent_task}}",$params);
		}
		unset($db);
	}		
	
	public static function deleteRecurrentTask($task_id='')
	{
		$task_id = (integer)$task_id;
		$db = new DbExt();
		$stmt="DELETE FROM {{recurrent_task}}
		WHERE task_id =".self::q($task_id)."
		";
		$db->qry($stmt);
		unset($db);
	}
	
	public static function getDriverByTokenInfo($token='')
	{
		if (empty($token)){
			return false;
		}
