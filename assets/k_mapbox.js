		...
		$.each( data , function( index, val ) {
			...
			if (!empty(val.lat)){
				...
				if ( map_hide_delivery==1){
		         ...
		         if(!empty(mapbox_marker[driver_id])){		         	
		         	mapbox_marker[driver_id].setLatLng([lat, lng]).update(); 
		         } else {
		         	mapbox_marker[driver_id] = L.marker([lat,lng], { icon : icon } ).addTo(mapbox);		           
		            mapbox_marker[driver_id].bindPopup(info_html);
		         }
