	 ...
	 success: function (data) {	  
	 	...
	 	if (data.code==1){
	 		...
	 		switch (action)
	 		{
	 			...
	 			case "removeServicesJson":
	 			  $(".json_preview").html('');
	 			  $(".json_remove_preview").remove();
	 			break;
...
$.validate({ 	
	...
	if ( $("#upload_services_json").exists() ){
		...
		var btn = document.getElementById('upload_services_json'),
        progressBar = document.getElementById('progressBar'),
        progressOuter = document.getElementById('progressOuter'),
        msgBox = document.getElementById('msgBox');
		...
		var uploader = new ss.SimpleUpload({
        button: btn,
        url: ajax_url+'/uploadJsonFile/?prefix=json',
        name: 'uploadfile',
        multipart: true,
        ...
        onComplete: function( filename, response ) {
            ...
            if ( response.success == 1 ) {                
                toast( jslang.success_uploaded );
                $(".json_preview").html(response.json_file);
            } else {
                if ( response.msg )  {                    
                    toast( response.msg );
                } else {                    
	...
	$( document ).on( "click", ".json_remove", function() { 
    	var a=confirm(jslang.delete_confirm+"?");
   	    if(a){
    	   callAjax("removeServicesJson",'' );
   	    }
    });		
