...
function callAjax(action,params,button)
{
	...
	ajax_request = $.ajax({
	  ...
	  beforeSend: function( xhr ) {		
	      if ((typeof  ajax_request !== "undefined") && ( ajax_request !== null)) {
	  	  	 ajax_request.abort();
	  	  	 busy(false,button);
	  	  } else {
	  	  	 busy(true,button);	
	...
	ajax_request.done(function( data ) {
		 ...
		 if (data.code==1){
	 		...
	 		switch (action)
	 		{
	 			...
	 			$(".recurrent_task_details").html( data.details.recurrent_html);
	 			...
	 			if ( !empty( data.details.task_lat )){
			        ...
			        switch (map_provider){
			       	   case "google":			       	
			       	      moveDeliveryMarkers( data.details.task_lat , data.details.task_lng );			       	      
			       	   break;
	 			...
	 			if ( !empty(data.details.drop_address)){
	 				...
	 				switch (map_provider){
	 					...
	 					case "google":	 		
	 					  movePickupMarkers( data.details.dropoff_task_lat,data.details.dropoff_task_lng );
       	                break;
       	                
       	                case "mapbox":
       	                  setTimeout(function(){
       	                     mapbox_plotMarkerPickup( data.details.dropoff_task_lat , data.details.dropoff_task_lng );
       	                  }, 500);
       	                break;
	 				}				    
	 			...
	 			fillRecurrentTask(data.details);
	 			...
	 			case "getContactInfo":
			       ...
			       switch (map_provider){
			       	  ...
			       	  case "mapbox":
			       	    mapbox_plotMarkerContact(data.details.addresss_lat , data.details.addresss_lng);
			       	  break;
			       }	 			  
	 			...
	 			case "loadContactInfo":
				   ...
				   switch (map_provider){
					  case "google":
					     moveDeliveryMarkers( data.details.addresss_lat , data.details.addresss_lng );					     
					  break;
					  
					  case "mapbox":
					    mapbox_plotMarkerDelivery(data.details.addresss_lat , data.details.addresss_lng );
					  break;
				   }
	 			...
	 			case "loadContactInfo2":
				  ...
				  switch (map_provider){
				    case "google":
				      movePickupMarkers( data.details.addresss_lat, data.details.addresss_lng );
				      //setMarkeyDropOffMap( $("#drop_address").val() ); 
				    break;
				  
				    case "mapbox":
				      mapbox_plotMarkerPickup(data.details.addresss_lat, data.details.addresss_lng );
				    break;
				  }
       ...
       switch (map_provider){
       	   case "google":
			   ...
			   .bind("geocode:result", function(event, result){	  
		  	      ...
		  	      moveDeliveryMarkers( result.geometry.location.lat() , result.geometry.location.lng() );		  	      
			   });	   
       	   break;
...
$(document).ready(function(){
	...
	if ( $("#layout_1").exists() ){
	   ...
	   setTimeout('getNotifications()', refresh_interval );
	   setTimeout('getSMSBalance()', refresh_interval+100 );
	}
...
function getInitialNotifications()
{
	 ...
	 notification_handle2 = $.ajax({
	  ...
	  crossDomain: true,
	  beforeSend: function( xhr ) {		
	      if ((typeof  notification_handle2 !== "undefined") && ( notification_handle2 !== null)) {
	  	  	 notification_handle2.abort();
	  	  } 				
	  }
	});
			
	notification_handle2.done(function( data ) {
    ...
    notification_handle2.always(function() {       
	   $ajax_sms_balance = null;   
    });
          
    /*FAIL*/
    notification_handle2.fail(function( jqXHR, textStatus ) {       	    
    });     	
}
...
{		
	...
	notification_handle = $.ajax({
	  ...
	  crossDomain: true,
	  beforeSend: function( xhr ) {					  	  
	  	  if ((typeof  notification_handle !== "undefined") && ( notification_handle !== null)) {	
	  	  	 notification_handle.abort();
	  	  } 
	  }
	});
			
	notification_handle.done(function( data ) {
    ...
    notification_handle.always(function() {        
    	notification_handle = null;   
    	clearInterval($cron_handle[1]);	  	 
    	$cron_handle[1] = setInterval(function(){getNotifications()}, refresh_interval );  
    });
          
    /*FAIL*/
    notification_handle.fail(function( jqXHR, textStatus ) {     
    	 clearInterval($cron_handle[1]);  	    
    });     	
...
{	
    ...
    switch (map_provider){
       ...
       map_dropoff_marker.addListener('dragend',function(event) {	      	 
   	  ...
   	  case "mapbox":
   	    mapbox_PlotMapPickup('map2_task',default_location_lat,default_location_lng);
	...
	if ( $(".contact_address").exists() ){    	    	    			
	   $(".contact_address").geocomplete({
	  	 country: default_country
	        ...
	        moveContactMarkers(result.geometry.location.lat(), result.geometry.location.lng());
	   });
    }
       ...
       switch (map_provider){
       	  ...
       	  $(".drop_address").geocomplete({
		  	 country: default_country
		  	   ...
		  	   $("#dropoff_task_lat").val( result.geometry.location.lat() );
		  	   $("#dropoff_task_lng").val( result.geometry.location.lng() );
		  	   
		  	   movePickupMarkers( result.geometry.location.lat(), result.geometry.location.lng() );
...
{		
	task_remaining_ajax = $.ajax({
	  ...
	  crossDomain: true,
	  beforeSend: function( xhr ) {	  	   
	      if ((typeof  task_remaining_ajax !== "undefined") && ( task_remaining_ajax !== null)) {
	  	  	 task_remaining_ajax.abort();
	  	  } 				
	  }
	});
			
	task_remaining_ajax.done(function( data ) {		
		 if (data.code==1){
    ...
    task_remaining_ajax.always(function() {        
    	  task_remaining_ajax = null;  
    	  clearInterval(task_remaining_handle);
    	  task_remaining_handle = setInterval(function(){getRemainingTask()}, refresh_interval+200 ); 	 
    });
          
    /*FAIL*/
    task_remaining_ajax.fail(function( jqXHR, textStatus ) {       	    
    	clearInterval(task_remaining_handle);
    });     	
}
...
{	
   ...
   switch (map_provider){
	    ...
	    map_contact_marker = map_contact.addMarker({
   	  ...
   	  case "mapbox":
   	    mapbox_PlotMap('map_contact',default_location_lat,default_location_lng);
...
$(document).ready(function(){
	...
	if ( $(".track-map").exists() ){	
		...
		switch (map_provider){
	   	  ...
	   	  case "mapbox":
	   	    mapbox_PlotMap('track-map',default_location_lat,default_location_lng);
	   	  break;
	   }   					
	}
...
{	
	...
	track_interval=setInterval(function(){
		...
		$(".track-details-wrap").append('<p>'+ jslang.lat+":"+ lat + " , " + jslang.lng +":"+ lng + '</p>');
...
$(document).ready(function(){
	...
	if ( $(".dashboard").exists() ){
		if(disabled_activity_tracking!=1){
		   activity_tracking = setInterval(function(){checkActivity()}, activity_tracking_interval );
		}
	}
...
checkActivity = function(){
	...
	ajax_activity = $.ajax({
	  ...
	  crossDomain: true,
	  beforeSend: function( xhr ) {     
	  	   if ((typeof  ajax_activity !== "undefined") && ( ajax_activity !== null)) {
	  	  	 ajax_activity.abort();
	  	  } 	    
      }
    });
...
$(document).ready(function(){
	...
	$( document ).on( "click", ".recurrent_task_enabled", function() {
		if($(this).is(':checked')){
			$(".recurrent_task_wrap_inner").show();
			addRequiredTask(true);
		} else {
			$(".recurrent_task_wrap_inner").hide();
			addRequiredTask(false);
		}
	});
	...
	if ( $(".jcalendar").exists() ){    	
	    $('.jcalendar').datetimepicker({	    
	        ...
	        onSelectDate:function(dp,$input){
	        	/*var date_formated=dp.format("YYYY-MM-DD");
	        	jcal = $input.parent();
	        	jcal.find("input").val( date_formated );*/
            }      
	    });	    	    	    
    }   
        
    $( document ).on( "click", ".recurrent_task_check_all", function() {
    	if($(this).is(':checked')){
    		$(".recurrent_task_days").prop( "checked", true );
    	} else {
    		$(".recurrent_task_days").prop( "checked", false );
    	}
    });
    $( document ).on( "click", ".recurrent_task_repetition_check_all", function() {
    	if($(this).is(':checked')){
    		$(".recurrent_task_repetition").prop( "checked", true );
    	} else {
    		$(".recurrent_task_repetition").prop( "checked", false );
    	}
    });    
...
addRequiredTask = function(required){
	if(required){
		$(".jcalendar").attr("required",true);
		$(".jcalendar2").attr("required",true);
		$(".recurrent_task_days").attr("data-validation","checkbox_group");
		$(".recurrent_task_days").attr("data-validation-qty","min1");
		$(".recurrent_task_days").attr("data-validation-error-msg", t("Please choose at least one") );
		
		/*$(".recurrent_task_repetition").attr("data-validation","checkbox_group");
		$(".recurrent_task_repetition").attr("data-validation-qty","min1");
		$(".recurrent_task_repetition").attr("data-validation-error-msg", t("Please choose at least one") );*/
		
		$(".recurrent_task_repetition").attr("data-validation","required");		
		$(".recurrent_task_repetition").attr("data-validation-error-msg", t("Please choose at least one") );
	} else {
		$(".jcalendar").removeAttr("required");
		$(".jcalendar2").removeAttr("required");
		$(".recurrent_task_days").removeAttr("data-validation");
		$(".recurrent_task_repetition").removeAttr("data-validation");
	}
};
...
fillRecurrentTask = function(data){
	dump("fillRecurrentTask");
	dump(data);
	if(data.recurrent_task_enabled==1){		
		addRequiredTask(true);
		$(".recurrent_task_enabled").prop( "checked", true );
		$(".recurrent_task_wrap_inner").show();		
		
		$(".jcalendar").val(data.recurrent_task_start_date);
		$(".jcalendar2").val(data.recurrent_task_end_date);
		
		$("#recurrent_task_start_date").val(data.recurrent_task_start_date);
		$("#recurrent_task_end_date").val(data.recurrent_task_end_date);

		recurrent_days = JSON.parse(rr_days);
		$.each( recurrent_days , function( day_key, day_val ) {
			if (data[day_val]==1){
				$(".rr_day_"+day_val ).prop( "checked", true );
			} else {
				$(".rr_day_"+day_val ).prop( "checked", false );
			}
		});
		
		recurrent_repetion = JSON.parse(rr_repetion);
		$.each( recurrent_repetion , function( repetion_key, repetion_val ) {
			if (data[repetion_val]==1){
				$(".rr_repetion_"+repetion_val ).prop( "checked", true );
			} else {
				$(".rr_repetion_"+repetion_val ).prop( "checked", false );
			}
		});
...
ResetRecurrentTask = function(){
	addRequiredTask(false);
	$(".recurrent_task_enabled").prop( "checked", false );
	$(".recurrent_task_wrap_inner").hide();		
};

getSMSBalance = function(){
	
	$ajax_sms_balance = $.ajax({
	  ...
	  crossDomain: true,
	  beforeSend: function( xhr ) {			  	  
	  	   if ((typeof  $ajax_sms_balance !== "undefined") && ( $ajax_sms_balance !== null)) {
	  	  	  $ajax_sms_balance.abort();
	  	   } 	
	  }
	});
			
	$ajax_sms_balance.done(function( data ) {
    ...
    $ajax_sms_balance.always(function() {        
    	$ajax_sms_balance = null;  
    	clearInterval($cron_handle[2]); 
    	$cron_handle[2] = setInterval(function(){getSMSBalance()}, refresh_interval ); 
    });
          
    /*FAIL*/
    $ajax_sms_balance.fail(function( jqXHR, textStatus ) {      
    	 clearInterval($cron_handle[2]);  	    
    });     	
